...
import { readBiddingItems } from '../utils/readBiddingItems';
...
test.describe('Bidding item ingestion', () => {
  test('Add bidding items from CSV', async ({ page }) => {
    // Long-running workflow (uploads + rich text + navigation)
    test.setTimeout(10 * 60 * 1000);

    const items = readBiddingItems();
    console.log(`\n▶ Adding new ${items.length} bidding items found`);
