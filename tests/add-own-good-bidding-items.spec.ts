...
import { readOwnGoodBiddingItems } from '../utils/readOwnGoodBiddingItems';
...
test.describe('Own Good Bidding item ingestion', () => {
  test('Add Own Good bidding items from CSV', async ({ page }) => {
    // Long-running workflow (uploads + rich text + navigation)
    test.setTimeout(10 * 60 * 1000);

    const items = readOwnGoodBiddingItems();
    console.log(`\n▶ Adding new ${items.length} own good bidding items found`);
